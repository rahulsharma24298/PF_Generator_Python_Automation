...
if st.session_state.pf_data is not None and st.session_state.resultset_data is not None:
    ...
    # ------ Step 2: ATM Selection Mode ------
    st.header("Step 2: Select ATM CE Tokens")

    atm_mode = st.radio("Choose ATM Selection Mode:", ("Select ATM Manually", "Select ATM by Range"), index=0)

    if atm_mode == "Select ATM Manually":
        ...
        with range_cols[0]:
            atm_exch = st.selectbox("Exchange", exchanges, key="range_atm_exch")
        with range_cols[1]:
            atm_segment = st.selectbox("Segment", segments, key="range_atm_segment")
        with range_cols[2]:
            atm_symbol = st.selectbox("Symbol", symbols, key="range_atm_symbol")
        with range_cols[3]:
            atm_expiry = st.selectbox("Expiry Date", expiries, key="range_atm_expiry")
        with range_cols[4]:
            atm_inst_type = st.selectbox("Instrument Type", inst_types, key="range_atm_inst_type")
        with range_cols[5]:
            atm_option_type = st.selectbox("Option Type", option_types, key="range_atm_option_type")
        with range_cols[6]:
            strike_gap = st.selectbox("Gap", [100, 50, 0], index=2, help="Select gap to filter ATM CE ending")

        start_range = st.number_input("Start Range (Whole Number Strike)", min_value=0, value=0, step=50)
        end_range = st.number_input("End Range (Whole Number Strike)", min_value=0, value=100000, step=50)

        def filter_atm_tokens_by_range(rs, exch, segment, symbol, expiry, inst_type, option_type, start_r, end_r, gap):
            filt = rs[(rs['Exch'] == exch) &
                      (rs['Segment'] == segment) &
                      (rs['Symbol'] == symbol) &
                      (rs['ExpiryDate_Readable'] == expiry) &
                      (rs['InstType'] == inst_type) &
                      (rs['OptionType'] == option_type) &
                      (rs['StrikePrice'] >= start_r) &
                      (rs['StrikePrice'] <= end_r)]

            if gap == 100:
                filt = filt[(filt['StrikePrice'] % 100000) == 10000]
            elif gap == 50:
                filt = filt[(filt['StrikePrice'] % 10000) == 5000]
            return filt

        if st.button("Find ATM CE Tokens in Range"):
            with st.spinner("Looking up ATM CE tokens in range..."):
                filtered_tokens_df = filter_atm_tokens_by_range(rs, atm_exch, atm_segment, atm_symbol, atm_expiry, atm_inst_type, atm_option_type, start_range, end_range, strike_gap)
                if filtered_tokens_df.empty:
                    st.warning("No ATM CE tokens found in the given range and gap criteria.")
                    st.session_state.atm_tokens_multi = {}
                else:
                    tokens_dict = pd.Series(filtered_tokens_df['Token'].values, index=filtered_tokens_df['StrikePrice']).to_dict()
                    st.success(f"Found {len(tokens_dict)} ATM CE tokens in range.")
                    st.session_state.atm_tokens_multi = tokens_dict
                    st.dataframe(filtered_tokens_df[['StrikePrice', 'Token']], height=200)
    ...
    if 'atm_tokens_multi' in st.session_state and st.session_state.atm_tokens_multi:
        ...
        with settings_cols[3]:
            cash_limit = st.number_input(
                "CASH_LIMIT (Rupees)", min_value=0, value=0, step=50,
                help="Filter rows where difference between ATM_CE and NONATM_CE strike is >= CASH_LIMIT and NONATM_CE strike is multiple of CASH_LIMIT. 0 disables filter."
            )
        ...
        def filter_by_cash_limit(df, cash_limit):
            if cash_limit == 0:
                return df.copy()
            filtered = []
            for _, row in df.iterrows():
                ...
                if len(parts) != 2:
                    continue
                atm_desc, nonatm_desc = parts

                def extract_strike(text):
                    m = re.search(r"(\\d+) CE$", text.strip())
                    return int(m.group(1)) if m else None

                atm_strike = extract_strike(atm_desc)
                nonatm_strike = extract_strike(nonatm_desc)
                if atm_strike is None or nonatm_strike is None:
                    continue

                diff = abs(nonatm_strike - atm_strike)
                if diff >= cash_limit and (nonatm_strike % cash_limit == 0):
                    filtered.append(row)
            if len(filtered) == 0:
        ...
        if generate_btn:
            ...
            if not output_data:
                ...
                # Apply CASH_LIMIT filtering here
                filtered_finalpf_df = filter_by_cash_limit(combined_output_df, cash_limit)

                # Apply strike gap filtering
                filtered_finalpf_df = filter_by_custom_gap(filtered_finalpf_df, strike_gap)

                if filtered_finalpf_df is not None and not filtered_finalpf_df.empty and 'Description' in filtered_finalpf_df.columns:
                    filtered_finalpf_df = filtered_finalpf_df.sort_values('Description', ascending=True).reset_index(drop=True)
                else:
                    st.warning("Filtered DataFrame is empty or missing 'Description' column.")
                    filtered_finalpf_df = pd.DataFrame(columns=combined_output_df.columns)

                if add_reverse_pf and 'OPCL' in filtered_finalpf_df.columns and not filtered_finalpf_df.empty:
                    reversed_rows = []
