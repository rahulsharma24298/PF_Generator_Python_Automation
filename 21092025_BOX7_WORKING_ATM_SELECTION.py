...
if st.session_state.pf_data is not None and st.session_state.resultset_data is not None:
    ...
    strike_cols = st.columns((1, 1, 1, 0.5))
    # Group strike price inputs in rows of 3 per line to save space
    for i in range(0, len(st.session_state.strike_prices), 3):
        s_cols = st.columns(3)
        for j in range(min(3, len(st.session_state.strike_prices) - i)):
            idx = i + j
            st.session_state.strike_prices[idx] = s_cols[j].number_input(
                f"Strike {idx + 1}",
                min_value=0,
                value=st.session_state.strike_prices[idx],
                key=f"strike_input_{idx}"
            )
    if strike_cols[3].button("Add Strike Price"):
    ...
    if 'atm_tokens_multi' in st.session_state and st.session_state.atm_tokens_multi:
        ...
        if generate_btn:
            ...
            for atm_strike, nonatm_df in nonatm_tokens_per_atm.items():
                ...
                for i, (_, row) in enumerate(nonatm_df.iterrows()):
                    ...
                    output_data.append({
                        ...
                        'Description': f"{atm_row.get('Name', 'ATM_CE')}|{nonatm_name}",
            ...
            if not output_data:
                ...
                # Sort by Description ascending A-Z
                filtered_finalpf_df = filtered_finalpf_df.sort_values('Description').reset_index(drop=True)

                # Add reverse PF rows if checked and OPCL column exists
                if add_reverse_pf and 'OPCL' in filtered_finalpf_df.columns:
                    ...
                    st.session_state.finalpf_df = combined_df
                else:
                    st.session_state.finalpf_df = filtered_finalpf_df

        # Show final output after generation
        if st.session_state.finalpf_df is not None:
            ...
            csv_data = st.session_state.finalpf_df.to_csv(index=False)
