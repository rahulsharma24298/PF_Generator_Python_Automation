...
Follow these steps:
1. Upload both files using the buttons below
2. Map the parameters using the dropdowns
3. Generate the final output file
""")
...
if st.session_state.pf_data is not None and st.session_state.resultset_data is not None:
    ...
    # ATM_CE Mapping section
    st.subheader("ATM_CE Mapping")
    col1, col2, col3 = st.columns(3)

    with col1:
        atm_exch = st.selectbox("Exchange", options=exchanges, key="atm_exch")
        atm_segment = st.selectbox("Segment", options=segments, key="atm_segment")
        atm_symbol = st.selectbox("Symbol", options=symbols, key="atm_symbol")

    with col2:
        atm_expiry = st.selectbox("Expiry Date", options=expiries, key="atm_expiry")
        atm_inst_type = st.selectbox("Instrument Type", options=inst_types, key="atm_inst_type")
        atm_option_type = st.selectbox("Option Type", options=option_types, key="atm_option_type")
    ...
    with col3:
        ...
        ] if atm_symbol and atm_expiry and atm_option_type else pd.DataFrame()
    ...
    # ATM_CE Mapping button functionality
    ...
    # NONATM_CE mapping, only if ATM token chosen
    ...
    if st.session_state.mapping_complete:
        ...
        if st.button("Generate Output File"):
            with st.spinner("Generating output file..."):
                ...
                csv = output_df.to_csv(index=False, sep='\t')
                expiry_date_str = str(atm_expiry_val).replace('/', '')
                filename = f"{expiry_date_str}_{st.session_state.download_count}.csv"
                st.download_button(
