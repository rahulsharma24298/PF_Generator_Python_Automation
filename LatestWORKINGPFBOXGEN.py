...
if st.session_state.pf_data is not None and st.session_state.resultset_data is not None:
    ...
    if hasattr(st.session_state, 'atm_token') and st.session_state.atm_token:
        ...
        with col3:
            st.write("Strike Range")
            nonatm_strike_min = st.number_input("Min Strike", value=int(st.session_state.atm_strike - 10000), key="nonatm_min")
            nonatm_strike_max = st.number_input("Max Strike", value=int(st.session_state.atm_strike + 10000), key="nonatm_max")
    ...
    # Step 3: Generate FinalPF.csv with strike gap filtering
    if st.session_state.mapping_complete and hasattr(st.session_state, 'nonatm_tokens_df'):
        ...
        # Build output mapping from tokens
        output_data = []
        ...
        # Strike gap input field and filtering as per your latest request
        ...
        def filter_by_custom_gap(df, gap):
            ...
            for idx, row in df.iterrows():
                ...
                if match:
                    ...
                    if gap == 100:
                        # Remove rows ending with 50
                        if last_two_digits != 50:
                            filtered_rows.append(row)
                    elif gap == 50:
                        # Remove rows ending with 00
                        if last_two_digits != 0:
                            filtered_rows.append(row)
                    else:
                        # For other gap values, keep all
                        filtered_rows.append(row)
                else:
                    filtered_rows.append(row)
